...
import { getClassworkTypeConfig } from '@lkp-parduli-rasa/domain';
...
export default function AdminClassroom() {
  ...
  const savePost = async () => {
    ...
    if (editingPostId) {
      await classroomDomain.updatePost(editingPostId, {
        title: postDraft.title,
        body: postDraft.body,
        updatedAt: now,
      });
    } else {
      await classroomDomain.createPost({
        courseId: activeClassroom.course.id,
        title: postDraft.title,
        body: postDraft.body,
  ...
  const togglePostPublish = async (postId) => {
    const post = activeClassroom.posts.find((item) => String(item.id) === String(postId));
    if (!post) {
      return;
    }

    await classroomDomain.updatePost(postId, {
      isPublished: post.isPublished === false,
      status: post.isPublished === false ? 'published' : 'draft',
      updatedAt: new Date().toISOString(),
    });
  };

  const deletePost = async (postId) => {
    await classroomDomain.removePost(postId);
  };

  const saveClassworkItem = async () => {
    const now = new Date().toISOString();
    await classroomDomain.createClassworkItem({
      courseId: activeClassroom.course.id,
      ...
      isPublished: true,
      order: (activeClassroom.genericItems || []).length + 1,
      createdAt: now,
      updatedAt: now,
    });
  ...
  const removeMember = (enrollmentId) => {
    classroomDomain.removeMember(enrollmentId);
  };

  const activateMember = (enrollmentId) => {
    classroomDomain.addMember(enrollmentId);
  };

  const verifyPayment = (enrollmentId) => {
    classroomDomain.verifyPayment(enrollmentId);
  };
