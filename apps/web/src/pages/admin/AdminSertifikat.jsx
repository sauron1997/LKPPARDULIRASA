...
import {
  ...
  useUpsertAdminCertificateMutation,
...
import { buildCertificateGate } from '@lkp-parduli-rasa/domain';
...
export default function AdminSertifikat() {
  ...
  const upsertCertificateMutation = useUpsertAdminCertificateMutation();
  const deleteCertificateMutation = useDeleteAdminCertificateMutation();
  ...
  const saveCertificate = async (event) => {
    ...
    try {
      ...
      const savedCertificate = await upsertCertificateMutation.mutateAsync({
        studentId: matchedStudent.student.id,
        payload: nextCertificate,
      });

      setSelectedCertificateId(savedCertificate?.id || null);
      setToast({
        tone: 'emerald',
  ...
  const deleteCertificate = async () => {
    ...
    await deleteCertificateMutation.mutateAsync(confirmDeleteId);
