...
export default function AdminDashboardCharts({
  ...
  const hasTrendData = registrationTrend.length > 0;
  const hasDistributionData = programDistribution.length > 0;
  ...
  return (
    <div className="grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)] xl:gap-7 2xl:gap-8">
      <AdminSectionCard
        ...
        description="Pergerakan jumlah pendaftar selama enam bulan terakhir, ditampilkan apa adanya ketika seri data tersedia."
        action={(
          <AdminTag tone={hasTrendData ? 'emerald' : 'amber'} className="px-3 py-1.5 text-sm">
            {hasTrendData ? `${trendLabel} vs awal periode` : 'Seri data belum tersedia'}
          </AdminTag>
        )}
      >
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#eef6f1] text-[#235845]">
            <TrendingUp size={20} />
          </div>
          <div>
            ...
            <p className="text-sm text-slate-500">
              {hasTrendData
                ? 'Momentum intake enam bulan terakhir'
                : 'Panel menahan visual tren sampai seri data benar-benar tersedia'}
            </p>
          </div>
        </div>

        <div className="mt-6 rounded-[28px] border border-[#d7e8df] bg-[linear-gradient(180deg,rgba(238,246,241,0.7),rgba(255,255,255,0.95))] p-4">
          <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-[270px] w-full lg:h-[286px]" role="img" aria-label="Grafik tren pendaftaran siswa baru">
            <defs>
              <linearGradient id={chartGradientId} x1="0" x2="0" y1="0" y2="1">
                <stop offset="0%" stopColor="#235845" stopOpacity="0.24" />
                <stop offset="100%" stopColor="#235845" stopOpacity="0.03" />
              </linearGradient>
            </defs>
            ...
            {areaChart.points.map((point) => (
              <g key={point.month}>
                ...
                <circle cx={point.x} cy={point.y} r="4" fill="#235845" />
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-500">
          <div className="flex items-center gap-2">
            <span className="inline-block h-3 w-3 rounded-full bg-[#235845]" />
            Jumlah pendaftaran
          </div>
          <span className="font-semibold text-slate-700">
            {hasTrendData
              ? `${latestValue} pendaftar pada periode terbaru`
              : 'Belum ada angka tren yang valid untuk ditampilkan'}
          </span>
        </div>
      </AdminSectionCard>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-6 lg:gap-7">
        <AdminSidebarPanel
          ...
          description="Program yang paling banyak dipilih siswa, atau state kosong bila komposisi belum tersedia."
          ...
          <div className="grid gap-5 xl:grid-cols-[210px_minmax(0,1fr)] xl:items-center">
            <div className="relative flex h-[210px] items-center justify-center">
              ...
              <div className="absolute flex h-[108px] w-[108px] flex-col items-center justify-center rounded-full bg-white shadow-[0_18px_45px_-20px_rgba(15,23,42,0.18)]">
                ...
                <span className="text-sm text-slate-500">{hasDistributionData ? 'Siswa aktif' : 'Snapshot'}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {!programDistribution.length ? (
                <div className="rounded-2xl border border-dashed border-[#f2d7b2] bg-[#fff8ef] px-4 py-6 text-sm text-slate-600">
                  Distribusi paket belum tersedia, jadi panel ini sengaja tidak menebak komposisi program.
                </div>
              ) : null}
            </div>
          </div>
