...
import {
  ...
  getPersistedAdminStudentBundle,
  getPersistedStudentPortal,
  listPersistedAdminStudentBundles,
  listPersistedCertificates,
  upsertPersistedCertificate,
  updatePersistedAdminStudent,
  ...
  return {
    ...
    async listStudents(filters = {}) {
      if (canUseDatabaseStudentPersistence()) {
        return listPersistedAdminStudentBundles(filters);
      }
    ...
    async getStudent(studentId) {
      if (canUseDatabaseStudentPersistence()) {
        const bundle = await getPersistedAdminStudentBundle(studentId);
    ...
    async updateStudent(studentId, payload = {}) {
      if (canUseDatabaseStudentPersistence()) {
        const updatedBundle = await updatePersistedAdminStudent(studentId, payload);
        ensure(updatedBundle, 'Data siswa tidak ditemukan.', 404, 'STUDENT_NOT_FOUND');
